...
#$ -pe smp 1
#$ -o /fast/AG_Akalin/asarigun/Arcas_Stage_1/FACTORY/RESULTS/TREAMID_HURI/job_outputs/
#$ -e /fast/AG_Akalin/asarigun/Arcas_Stage_1/FACTORY/RESULTS/TREAMID_HURI/job_errors/
#$ -t 150001-180374 # Adjust this range to the number of lines in "run.lst" - check "wc -l run.lst"
#              # for testing, only try a small range like "-t 1-10" - when this worky nicely,i
#              # then for full computation select the remaining range, e.g. "-t 11-180374"
...
SCRIPT=$(sed "${SGE_TASK_ID}q;d" < /fast/AG_Akalin/asarigun/Arcas_Stage_1/FACTORY/RUNS/TREAMID_HURI/run.lst)
