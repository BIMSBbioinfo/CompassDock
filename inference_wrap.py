...
def run_docking(args, ligand_description = None):
    """
    ...
    # Attempt to load the configuration from the YAML file specified in args.config
    if args.config:
        try:
            # Directly use the file-like object, since args.config is already one
            args.config.seek(0)
        ...
        print("No configuration file provided. Please specify a configuration file using --config.")
        return

    # Proceed with updating arg_dict using config_dict
    arg_dict = args.__dict__
    for key, value in config_dict.items():
        if key in arg_dict and isinstance(arg_dict[key], list) and isinstance(value, list):
    ...
    if args.protein_ligand_csv is not None:
        ...
        if ligand_description is None:
            print(f'Using initial ligand SMILES: {args.ligand_description}')
            ligand_description_list = [args.ligand_description]
        else:
            print(f'Using predocked ligand at {ligand_description}')
    ...
    print(f'Successful dockings: {results_summary["success_count"]}')
    print(f'Results are in {args.out_dir}')

    return results_summary, protein_path_list, write_dir, args, best_confidence_score
