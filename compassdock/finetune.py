...
from compassdock.compassdock.training import train_epoch, test_epoch, loss_function, inference_epoch_fix
from DiffDock.utils.utils import save_yaml_file, get_optimizer_and_scheduler, get_model, ExponentialMovingAverage
from compassdock.compassdock.esm_processing import create_combined_parser, process_protein_data, extract_representations, load_and_save_embeddings
...
if __name__ == '__main__':
    ...
    #esm_parser, finetune_args = create_combined_parser(), parse_train_args()
    #esm_args = esm_parser.parse_args()
    args = parse_train_args()
    if not os.path.exists(args.out_file):
        print(f"The specified output file {args.out_file} does not exist. Creating a new one...")
        process_protein_data(args)
        extract_representations(args)
        load_and_save_embeddings(args)
    
    #finetune_args = parse_train_args()
    main_function(args)
